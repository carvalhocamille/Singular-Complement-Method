...
muP = 9;                             %Value set to match the computed singular exponant
muM = 9; 
...
[KK,MM,SS,~]=matrix_assembly(Coorneu,Numtri,Reftri,Numaretes,Refaretes,Nbaretes,epsilonP,epsilonM,muP,muM);
% =====================================================
%% ----------------Right-hand side---------------------
% =====================================================
FF = MM*F /muP; %MM is weigthed by mu
...
figure;
 trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),real(eta_h));
 colorbar;
 view(2);shading interp;grid off; axis 'equal';
 title('$\eta_h$ on the mesh','Interpreter','latex')

figure;
title('Localized Singularities')
subplot(2,2,1)
 trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),real(Zsinguh));
 colorbar;
 view(2);shading interp;grid off; axis 'equal';
 title('$ Re(\eta s_h) $','Interpreter','latex')
 subplot(2,2,2)
 trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),real(Zsinguh_b));
 colorbar;
 view(2);shading interp;grid off; axis 'equal';
 title('$ Re(\eta \overline{s_h}) $','Interpreter','latex')
 subplot(2,2,3)
 trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),imag(Zsinguh));
 colorbar;
 view(2);shading interp;grid off; axis 'equal';
 title('$ Im(\eta s_h) $','Interpreter','latex')
 subplot(2,2,4)
 trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),imag(Zsinguh_b));
 colorbar;
 view(2);shading interp;grid off; axis 'equal';
 title('$ Im(\eta \overline{s_h}) $','Interpreter','latex')
...
subplot(2,2,1)
 ...
 colorbar;
 view(2);shading interp;grid off; axis 'equal'; 
 title('$\Re div(\sigma \nabla s)_h$','Interpreter','latex')
 subplot(2,2,2)
 trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),real(div_s_grad_sh_b));
 colorbar;
 view(2);shading interp;grid off; axis 'equal'; 
 title('$\Re div(\sigma \nabla \overline{s})_h$','Interpreter','latex')
 subplot(2,2,3)
 trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),imag(div_s_grad_sh));
 colorbar;
 view(2);shading interp;grid off; axis 'equal'; 
 title('$\Im div(\sigma \nabla s)_h$','Interpreter','latex')
 subplot(2,2,4)
 trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),imag(div_s_grad_sh_b));
 colorbar;
 view(2);shading interp;grid off; axis 'equal'; 
 title('$\Im div(\sigma \nabla \overline{s})_h$','Interpreter','latex')
...
[BdS,BdSb] = NCQ(Coorneu,Numtri,Reftri,div_s_grad_sh,Zsinguh,div_s_grad_sh_b,Zsinguh_b,muM,muP);

%Create system to solve to find the dual singularity
AA1(:,1:end-1)= - KK(interior,interior1) +  MM(interior,interior1);    %Matrix for regular part of the dual singularity, z_h
...
figure; 
spy(AA1);
title('Matrix for solving for the dual singularity')

figure;
title('Coupling terms')
subplot(2,2,1)
trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),real(BdSb));
colorbar;
view(2);shading interp;grid off; axis 'equal'; 
title('$ \Re \beta(\zeta \overline{s}, w_h)$','Interpreter','latex')
subplot(2,2,2)
trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),real(BdS));
colorbar;
view(2);shading interp;grid off; axis 'equal'; 
title('$\Re \beta(\zeta {s}, w_h)$','Interpreter','latex')
subplot(2,2,3)
trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),imag(BdSb));
colorbar;
view(2);shading interp;grid off; axis 'equal'; 
title('$ \Im \beta(\zeta \overline{s}, w_h)$','Interpreter','latex')
subplot(2,2,4)
trisurf(Numtri2,Coorneu(:,1),Coorneu(:,2),imag(BdS));
colorbar;
view(2);shading interp;grid off; axis 'equal'; 
title('$ \Im \beta(\zeta {s}, w_h)$','Interpreter','latex')
...
%% 6) Compute the singularity coefficient (b)
% =====================================================
 [rhs,~] = NCQ(Coorneu,Numtri,Reftri,Dual_singularity,zeros(Nbpt,1),zeros(Nbpt,1),zeros(Nbpt,1),muM,muP);

 % Compute int sigma Phi^2 using Periodic Trapezoid Rule
 Nsteps =200;
 di = 2*pi/(Nsteps-1);
 value = 0;
 for k=1:Nsteps
     i = -pi + 2*pi/(Nsteps-1) *(k-1); 
     value = value + di * sPhi(i)*Phi(i);
 end
